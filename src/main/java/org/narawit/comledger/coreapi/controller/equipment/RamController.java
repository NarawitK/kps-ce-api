...
import org.narawit.comledger.coreapi.service.equipment.RamService;
...
public class RamController {
	...
	private final RamService svc;
	
	public RamController(RamService svc) {
		this.svc = svc;
	}
	
	@GetMapping
	public ResponseEntity<BaseResponse<Iterable<RamContract>>> getAll(){
		return ControllerHelper.getOkResponseEntity(FETCH_SUCCESSFULLY, svc.findAll());
	}
	
	@GetMapping("/{id}")
	public ResponseEntity<BaseResponse<RamContract>> getById(@PathVariable Long id){
		return ControllerHelper.getOkResponseEntity(FETCH_SUCCESSFULLY, svc.findById(id));
	}
	
	@PostMapping("/add")
	public ResponseEntity<BaseResponse<RamContract>> add(@Valid @RequestBody RamRequest req){
		try {
	...
	@PutMapping("/edit/{id}")
	public ResponseEntity<BaseResponse<RamContract>> edit(@PathVariable Long id, @Valid @RequestBody RamRequest req){
		try {
	...
	public ResponseEntity<BaseResponse<RamContract>> remove(@PathVariable Long id){
		svc.remove(id);
