...
import org.narawit.comledger.coreapi.service.equipment.UpsService;
...
public class UpsController {
	...
	private final UpsService svc;
	
	public UpsController(UpsService svc) {
		this.svc = svc;
	}
	
	@GetMapping
	public ResponseEntity<BaseResponse<Iterable<UpsContract>>> getAll(){
		return ControllerHelper.getOkResponseEntity(FETCH_SUCCESSFULLY, svc.findAll());
	}
	
	@GetMapping("/{id}")
	public ResponseEntity<BaseResponse<UpsContract>> getById(@PathVariable Long id){
		return ControllerHelper.getOkResponseEntity(FETCH_SUCCESSFULLY, svc.findById(id));
	}
	
	@PostMapping("/add")
	public ResponseEntity<BaseResponse<UpsContract>> add(@Valid @RequestBody UpsRequest req){
		try {
	...
	@PutMapping("/edit/{id}")
	public ResponseEntity<BaseResponse<UpsContract>> edit(@PathVariable Long id, @Valid @RequestBody UpsRequest req){
		try {
	...
	public ResponseEntity<BaseResponse<UpsContract>> remove(@PathVariable Long id){
		svc.remove(id);
